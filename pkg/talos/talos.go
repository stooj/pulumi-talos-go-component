...
func NewTalosCluster(
	ctx *pulumi.Context,
	name string,
	talosClusterArgs TalosClusterArgs,
	...
	talosCustomImage, err := digitalocean.NewCustomImage(
		ctx,
		"talosCustomImage",
		&digitalocean.CustomImageArgs{
			...
			Url: pulumi.Sprintf(
				"https://factory.talos.dev/image/376567988ad370138ad8b2698212367b8edcb69b5fd68c80be1f2ec7d603b4ba/%s/digital-ocean-amd64.raw.gz",
				talosClusterArgs.Version,
			),
		},
	)
	if err != nil {
	...
	for i := 0; i < talosClusterArgs.CountControlPlane; i++ {
		controlPlane, err := digitalocean.NewDroplet(
			ctx,
			fmt.Sprintf("controlPlane-%d", i),
	...
	for i := 0; i < talosClusterArgs.CountWorker; i++ {
		worker, err := digitalocean.NewDroplet(
			ctx,
	...
	for i := 0; i < talosClusterArgs.CountControlPlane; i++ {
		_, err := machine.NewConfigurationApply(
			ctx,
			fmt.Sprintf("cpConfigApply-%d", i),
	...
	for i := 0; i < talosClusterArgs.CountWorker; i++ {
		_, err := machine.NewConfigurationApply(
			ctx,
			fmt.Sprintf("workerConfigApply-%d", i),
			&machine.ConfigurationApplyArgs{
				ClientConfiguration:       machineSecrets.ClientConfiguration,
